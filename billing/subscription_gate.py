...
# key: customer_id -> (expires_at, result_dict)
...
def _is_subscription_active_status(status: str | None) -> bool:
...
def _check_subscription_via_stripe(customer_id: str) -> SubscriptionCheckResult:
    ...
    if cached:
        ent = None
        if isinstance(cached.get("entitlements"), dict):
            # Rehydrate Entitlements safely
            e = cached["entitlements"]
            ent = Entitlements(plan=str(e.get("plan", "starter")), features=dict(e.get("features", {})), limits=dict(e.get("limits", {})))
        return SubscriptionCheckResult(
    ...
    try:
        ...
        if not best:
            payload = {"active": False, "reason": "no_subscription", "subscription_id": None, "price_id": None, "status": None, "entitlements": None}
        ...
        # Conservative failure: if Stripe fails, treat as not active when gating is ON
...
def require_active_subscription(_fn: Optional[F] = None, *, feature: Optional[str] = None) -> F:
    ...
    def decorator(fn: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if feature and result.entitlements and not feature_enabled(result.entitlements, feature):
                return jsonify(
                    {

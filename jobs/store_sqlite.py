...
def _db() -> sqlite3.Connection:
    ...
    # NOTE:
    # - timeout helps with transient writer locks
    # - check_same_thread=False allows safe use across threads when the app uses threadpools,
    #   but we still open a NEW connection per call to avoid sharing a single connection.
...
if __name__ == "__main__":
    ...
    if r.should_process is not True:
        raise SystemExit("record_webhook_event: expected should_process=True for new event")

    r2 = record_webhook_event(event_id=eid, event_type="test.event", payload={"id": eid}, livemode=False)
    if r2.should_process is not False:
        raise SystemExit("record_webhook_event: expected should_process=False for duplicate event")
    ...
    if jid != jid2:
        raise SystemExit("enqueue_job: expected same job_id for duplicate idempotency_key")
